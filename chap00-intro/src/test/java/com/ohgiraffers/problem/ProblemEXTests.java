...
public class ProblemEXTests {
    ...
    /* title. 테스트 코드 기반으로 JPA 를 사용하지 않을 때 생기는 문제 EX */
    ...
    *  */
    ...
    // 테스트 전 con 생성 테스트 후 con 롤백 및 종료
    @BeforeEach
    ...
    /* comment.
    *   JDBC 를 직접적으로 사용시 문제점 확인.
    *   1. 데이터 변환, SQL 문 작성, JDBC 코드 중복 작성 문제
    *   - 개발 시간 증가, 불필요한 코드 많음, 유지보수성 악화
    *   2. SQL 에 의존적인 개발
    *  */

    /* index. 1번 문제확인 (바로 위 comment 내용) */
    @Test
    @DisplayName("직접 SQL 문을 작성해서 메뉴 조회시 문제 확인")
    void testDirectSQL() throws SQLException {
        ...
        * */
        ...
        // assertNull : null 값을 가져오고 싶은데 menudb 에는 null 값이 아니다.
        Assertions.assertNotNull(menuList);

        // 반복하는 menuList 를 menu 에 담아 출력해주는 것
    ...
    *  */
    ...
    /* index. 3. 패러다임의 불일치 (객체 지향 관점에서 벗어남, 상속, 다형성 , 캡슐화, 추상화) */
    /* comment.
    *   SQL 관점에서의 1:N -> 엔티티 간의 연관성을 FK로 가지고 있음
    *   JAVA 관점에서의 1:N -> 연관성을 자신의 멤버(필드)로 가지게 된다.
    *  */

    /* index. 4. 동일성 보장의 문제 */
    // 하나의 인스턴스를 가지고 수정한게 반영되도록 싱글톤
    // 컨트롤러할때 빈의 디폴트는 싱글톤으로 관리된다.
    // 어찌보면 DB 또한 마찬가지로 누군가가 수정하면 다른 사람도 알아야해요
    // 엔티티도 자바에서 빈처럼 관리 하나의 ROW 는 하나의 인스턴스로 관리
    // update 없이 set 으로 필드 수정하면 반영
    @Test
    ...
    void testEquals() throws SQLException {
        ...
        // 값은 똑같을지 몰라도 실제 보관하고 있는 주소는 다르다.
        // db 는 데이터창고인데 똑같은 row 를 출력했는데 다르게 나온다 말이 안됨
        // 추출했을때 단 하나의 row 로 구분이 되야해요
        // 하나의 row 는 똑같해야 합니다. 그래야 수정되면 바로바로 반응되죠.
        ...
        *  */
        ...
        // assertFalse(menu1 번과 menu 2번이 다르다 내부가 false 여야 true)
